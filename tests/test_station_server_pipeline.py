...
def write_five_csvs(root: Path) -> None:
    times = pd.date_range("2024-06-01 06:00:00", periods=288, freq="15min")
...
def test_five_csv_server_pipeline_is_two_modal_and_five_horizon(tmp_path: Path) -> None:
    ...
    config["data"].update({
        "root_dir": str(raw), "processed_dir": str(processed),
        "history_length": 4, "train_fraction": 0.6, "val_fraction": 0.2,
    })
    config["model"].update({
